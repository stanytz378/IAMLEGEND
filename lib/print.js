...
// Safe timezone – avoids "Invalid time zone" errors
const safeTimeZone = (() => {
    try {
        new Date().toLocaleTimeString('en-US', { timeZone: settings.timeZone || 'Africa/Nairobi' });
        return settings.timeZone || 'Africa/Nairobi';
    } catch (e) {
        console.warn('⚠️ Invalid timezone specified, falling back to UTC');
        return 'UTC';
    }
})();
...
async function printMessage(message, sock) {
    try {
        ...
        const timeStr = timestamp.toLocaleTimeString('en-US', { 
            ...
            hour12: false, timeZone: safeTimeZone
        });
        ...
        console.log(
            ...
            chalk.magenta(displayType) + chalk.gray(fileSizeStr)
        );
        ...
        if (messageText) {
            ...
            const isBotResponse = messageText.includes('ᴵ ᴬᴹ ᴸᴱᴳᴱᴺᴰ') || 
            ...
            console.log(
                ...
                (isCommand ? chalk.greenBright(displayText) : 
                 isBotResponse ? chalk.cyan(displayText) : 
                 fromMe ? chalk.blueBright(displayText) : chalk.white(displayText))
            );
        }
...
function printLog(type, message) {
    ...
    const colors = {
        ...
        error: chalk.red, connection: chalk.cyan, store: chalk.magenta
    };
    const icons = {
    ...
    console.log(chalk.gray(`[${timestamp}]`) + ' ' + color(icon) + ' ' + color(message));
}
